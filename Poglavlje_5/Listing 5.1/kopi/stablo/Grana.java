...
/*
			if (DFAEditor.mode == DFAEditor.MOVE | DFAEditor.mode == DFA_NFA_Reg.MOVE) {
				g.drawLine(x3-3,y3-3,x3+3, y3+3);
				g.drawLine(x3+3,y3-3,x3-3, y3+3);
				g.drawLine(x4-3,y4-3,x4+3, y4+3);
				g.drawLine(x4+3,y4-3,x4-3, y4+3);
			}
*/
			g.setColor(Color.blue);
...
/*
		  if ((x+xCor(i1,aDir+.5)) > x){
			  g2d.drawString(""+tezina, x+xCor(i1,aDir+.5),y+yCor(i1,aDir+.5));
		  } else{
			  g2d.drawString(""+tezina, x+xCor(i1,aDir-.5),y+yCor(i1,aDir-.5));
		  }
*/
