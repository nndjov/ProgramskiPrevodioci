	...
	Glavni(){
        ...
        JButton start = new JButton("Skener");
           ...
           start.addActionListener( new ActionListener() {
                public void actionPerformed(ActionEvent e){
					//Reader r = new StringReader(ulazT.getText().trim());
					Reader r = new StringReader("2+3;");
					Yylex lexer = new Yylex(r);
					try {
						 Symbol token = lexer.next_token();
						 while( token != sym.EOF) {
							 REZULTAT = REZULTAT + " " + token;
        ...
        /////////////////////
           ...
           parser.addActionListener( new ActionListener() {
				...
				try{
					/*
					  InputStream targetStream = new ByteArrayInputStream(ulazT.getText().getBytes());
					  InputStreamReader in = new InputStreamReader(targetStream);
					  BufferedReader input = new BufferedReader(in);
					  */
		  			Reader r = new StringReader(ulazT.getText().trim());
		  			System.out.println(ulazT.getText());
		  			//Reader r = new StringReader("2+3;3+4;");
			 		parser p = new parser( new Yylex(r));
			  		p.parse();
			  		monitorT.setText(Glavni.REZULTAT);


			  		//Object rez = p.parse().value;
			  		//Symbol s = p.debug_parse();
			  		//System.out.println("rez="+s);

					  //parser par = new parser( new Yylex(input));
					  //par.parse();
					  //Symbol s =par.debug_parse();
					  //Object result = par.parse().value;
				}catch(Exception ex){}
